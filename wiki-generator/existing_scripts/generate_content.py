...
class WikiContentGenerator:
    ...
    def __init__(self, config_path: str = "pages.yaml", api_key: Optional[str] = None, content_format: Optional[str] = None, link_targets: Optional[list] = None):
        ...
        if self.content_format in ('miraheze', 'mediawiki', 'fandom', 'telepedia'):
            self.content_format = 'mediawiki'
        self.link_targets = link_targets or []
    ...
    def _build_link_bank_prompt(self) -> str:
        """Build a prompt section for the link bank."""
        if not self.link_targets:
            return ""

        prompt = "\n## Link Bank\n"
        prompt += "The following external links should be incorporated naturally into the content where contextually relevant.\n"
        prompt += "Rules:\n"
        prompt += "- Use the EXACT URLs provided — never modify them.\n"
        prompt += "- Only use the allowed anchor text options listed for each URL.\n"
        prompt += "- Each link has a global target count — the total number of times it should appear across ALL generated pages combined, not per page. Include it in this page only if it fits naturally.\n"
        prompt += "- Prioritize readability over forcing links. If a link does not fit naturally in this page, skip it entirely.\n\n"

        for entry in self.link_targets:
            url = entry.get('url', '')
            anchors = entry.get('anchors', [])
            count = entry.get('count', 1)
            if url and anchors:
                anchor_list = ', '.join(f'"{a}"' for a in anchors)
                prompt += f"- URL: {url}\n  Allowed anchors: {anchor_list}\n  Global target count: {count}\n"
    ...
    def generate_page(self, page: dict) -> str:
        ...
        user_prompt += self._build_link_bank_prompt()
